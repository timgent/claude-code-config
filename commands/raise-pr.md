...
2. Checking at .github/pull_request_template.md if there is a template to follow and if so following that template when raising the PR
  1. NOTE: If there is no template then just include a "What this PR does" heading, a "How to review this PR" heading, and a "Manual testing steps" heading
  2. NOTE: If there is a template, add "How to review this PR" as its own heading after the section that describes what the PR does, rather than forcing it into an existing template slot
...
as succinct as possible!
  ...
  3. Writing the "How to review this PR" section for a reviewer who has not seen the code:
    1. Give the reading order — which file or area to start with, what to read next, and why that order
    2. Decide the handful of things a reviewer must understand about the change *as a whole* first. Do not walk the diff looking for things to say
    3. The bar: a reviewer who skipped it could still approve this PR, but would later make a *wrong change* — add the next case in the wrong layer, cross the new boundary, break an invariant that lives in another file. Anything true from reading the diff alone does not clear it
    4. Where the description has a diagram, point at it rather than re-explaining the shape
    5. Keep it to one sentence for a single-file change and at most five bullets otherwise. It is not a second "What this PR does"
